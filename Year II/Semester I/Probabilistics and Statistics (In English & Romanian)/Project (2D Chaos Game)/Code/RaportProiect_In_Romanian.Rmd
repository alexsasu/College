---
title: "Raport 'Chaos Game Bidimensional'"
author:
- name: Constantin Filip-Damian
- name: Pătulea Maria-Eliza
- name: Sasu Alexandru-Cristian
  affiliation: Grupa 242
date: '2022-01-28'
...
Tema proiectului ales de echipa noastră este tema nr. 5, aceea de a realiza o aplicație web (folosind pachetul `shiny` pentru `R` dezvoltat de `RStudio`) care să implementeze **Chaos Game** în două dimensiuni.

### II Ce reprezintă **Chaos Game**?

În matematică, termenul de Chaos Game se referă la o metodă de a crea un [fractal](https://ro.wikipedia.org/wiki/Fractal), folosind un poligon și un punct inițial selectat la întâmplare în interiorul acestuia. Fractalul este format prin crearea iterativă a unei secvențe de puncte, începând cu punctul aleatoriu inițial, în care fiecare punct din secvență este o fracție dată a distanței dintre punctul anterior și unul dintre vârfurile poligonului; vârful este ales la întâmplare în fiecare iterație.

Repetarea acestui proces iterativ de un număr mare de ori, selectând vârful la întâmplare la fiecare iterație și lăsând la o parte primele câteva puncte din secvență, va produce adesea (dar nu întotdeauna) o formă fractală. Folosind un triunghi obișnuit și factorul $\frac{1}{2}$ va rezulta triunghiul Sierpinski, în timp ce crearea aranjamentului adecvat cu patru puncte și un factor $\frac{1}{2}$ va crea o afișare a unui *Tetraedru Sierpinski*, analogul tridimensional al triunghiului Sierpinski. Pe măsură ce numărul de puncte este crescut la un număr N, aranjamentul formează un [Simplex](https://en.wikipedia.org/wiki/Simplex) Sierpinski (N-1)-dimensional corespunzător.

### III Crearea aplicației web

În procesul de realizare al aplicației, am utilizat următoarele pachete:  
- `shiny` -> este un pachet care oferă un web framework elegant și puternic, pentru realizarea aplicațiilor web folosind `R`.  
- `shinyBS` -> este un pachet care adaugă componente ce aparțin framework-ului Bootstrap, adiționale pachetului `shiny`; funcții utilizate: `bsButton`, `updateButton`.  

Pentru a crea aplicația, am folosit funcția `shinyApp()` din pachetul `shiny`, având drept argumente definiția `UI` a aplicației (un apel către `fluidPage()`), `server`-ul, care este o funcție cu 3 parametrii (`input`, `output` și `session`), și o listă `options`, prin intermediul căreia am specificat dimensiunile în pagină ale aplicației.

În plus, figurile geometrice incluse în acest Chaos Game sunt generate prin apelul unor funcții, ce fololsesc input-ul ales de către utilizator.  
Spre exemplu, pentru a figura un triunghi (*Triunghiul lui Sierpinski*, mai specific), utilizatorul alege un număr de puncte (între 1 și 50,000) ce vor forma figura, precum și o valoare pentru raportul distanței față de punctul de start (între 0.01 și 1). Funcția destinată acestui triunghi este următoarea, și este insoțită de o serie de comentarii:
...
Triunghiul lui Sierpinski este un fractal care poartă numele matematicianului polonez Wacław Sierpiński, și care poate fi creat pornind de la un triunghi echilateral și tăind repetat triunghiuri mai mici din centrul său. 

```{r, echo = FALSE, out.width = "30%", fig.cap = "**Triunghiul lui Sierpinski**", fig.align = 'center'}
knitr::include_graphics("sierpinski_triangle.png")
```

Pentru a genera următoarele figuri geometrice, am utilizat funcțiile descrise mai jos, care au un comportament similar cu cel al funcției destinate triunghiului.  

- Pătrat (*Covorul lui Sierpinski*) -> un fractal plan descris pentru prima dată de Wacław Sierpiński în 1916, realizat prin tehnica de subîmpărțire a unui pătrat în copii mai mici ale lui, eliminând una sau mai multe copii și continuând recursiv.

```{r, echo=FALSE, out.width = "30%", fig.cap = "**Covorul lui Sierpinski**", fig.align = 'center'}
knitr::include_graphics("sierpinski_carpet.png")
```
...
- Pătrat restricționat -> restricția acestei figuri este că vârful curent procesat nu poate fi la un punct distanță, în sensul acelor de ceasornic, de vârful ales anterior.

```{r, echo=FALSE, out.width = "30%", fig.cap = "**Pătrat restrictionat**", fig.align = 'center'}
knitr::include_graphics("restricted_square.png")
```
...
- Pentagon restricționat -> restricția aplicată pentagonului este că vârful curent procesat nu poate fi ales în următoarea iterație.

```{r, echo=FALSE, out.width = "30%", fig.cap = "**Pentagon restrictionat**", fig.align = 'center'}
knitr::include_graphics("restricted_pentagon.png")
```
...
- Hexagon (*Hexagonul lui Sierpinski*) -> această figura se construiește astfel: în primul rând, se ia un poligon echilateral cu șase laturi. Apoi, în fiecare colț se înscrie un nou hexagon echilateral, și acum fiecare latură este o treime din hexagonul original. În continuare, se șterge hexagonul anterior și se repetă recursiv acest proces. Cu fiecare iterație recursivă se desenează hexagoane de dimensiuni mai mici și pentru că sunt întotdeauna situate în colțurile hexagonului anterior, se păstrează forma și dimensiunea figurii inițiale.

```{r, echo=FALSE, out.width = "30%", fig.cap = "**Hexagonul lui Sierpinski**", fig.align = 'center'}
knitr::include_graphics("sierpinski_hexagon.png")
```
...
- *Barnsley (Cyclosorus) Fern* -> un fractal numit după matematicianul britanic Michael Barnsley, care l-a descris pentru prima dată în cartea sa *Fractals Everywhere*. Deși, teoretic, *Barnsley Fern* ar putea fi trasat manual cu un pix și hârtie milimetrică, numărul de iterații necesare ajunge la zeci de mii, ceea ce face ca utilizarea unui computer să fie obligatorie. Atât timp cât partea matematica este programată corect folosind matricea de constante a lui Barnsley, se va produce aceeași formă de ferigă.

```{r, echo=FALSE, out.width = "30%", fig.cap = "**Barnsley (Cyclosorus) Fern**", fig.align = 'center'}
knitr::include_graphics("barnsley_fern.png")
```
...
### IV Codul aplicației `shiny`

După cum am menționat mai sus, am creat aplicația web folosind funcția `shinyApp()`, iar codul aferent acesteia este:
...
shinyApp(
  ui <- fluidPage(
    ...
    p("This two dimensional version of the Chaos Game consists of a set of
      ...
      closer the fractals get to the center of the plot."),
  ...
  options = list(height = 1000, width = 925)
)
```

### V Interfața aplicației
...
###################################
# Defining the application inline # 
...
În realizarea acestei aplicații web în `shiny`, dar și a acestui raport în `R Markdown`, am folosit următoarele resurse:

- [Pagina de Wikipedia pentru *Barnsley Fern*](https://en.wikipedia.org/wiki/Barnsley_fern)
- [Pagina de Wikipedia pentru *Chaos Game*](https://en.wikipedia.org/wiki/Chaos_game)
- [Chaos Game -- 2 Dimensions: Shiny app](https://gist.github.com/calpolystat/d40a02fa87508ac5ac4b)
- [Chaos game to get specific fractal - Mathematics Stack Exchange](https://math.stackexchange.com/questions/2583781/chaos-game-to-get-specific-fractal?rq=1)
- [Regular Pentagon -- from Wolfram MathWorld](https://mathworld.wolfram.com/RegularPentagon.html)
- [Sierpiński triangle](https://overlordoftheuberferal.com/tag/sierpinski-triangle/)
- [How can you find the coordinates in a hexagon? - Quora](https://www.quora.com/How-can-you-find-the-coordinates-in-a-hexagon)
- [Introduction to R Markdown](https://rmarkdown.rstudio.com/articles_intro.html)
- [R Markdown: The Definitive Guide](https://bookdown.org/yihui/rmarkdown/)
- [Add Images to an R Markdown Report | Earth Data Science - Earth Lab](https://www.earthdatascience.org/courses/earth-analytics/document-your-science/add-images-to-rmarkdown-report/)
